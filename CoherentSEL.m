...
%BIT DECISION 
if(abs(h1)>abs(h2)) %Selection Rule
b_dec=sign(r1);
error_q= error_q+qfunc(sqrt((abs(h1)^2*2*P/N0))); %Accumulate Ins. BER from Ch1
else
    error_q= error_q+qfunc(sqrt((abs(h2)^2*2*P/N0))); %Accumulate Ins. BER from Ch2
